...
class ResNet15_v1(nn.Module):
    ...
    def summary(self, batch_size:int=-1):
            device = str(next(self.parameters()).device) # Get the device of the model
...
class ResNet15_v2(nn.Module):
    def __init__(self, num_classes:int=2):
        ...
        self.flatten = nn.Flatten()
        self.fc = nn.Linear(in_features=512*2, out_features=num_classes)
        self.softmax = nn.LogSoftmax(dim=-1)
